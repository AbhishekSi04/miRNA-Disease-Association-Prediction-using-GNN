...
# ROC Curve
fpr, tpr, _ = roc_curve(label_all, y_score_all)
roc_auc = auc(fpr, tpr)
...
precision, recall, _ = precision_recall_curve(label_all, y_score_all)
avg_precision = average_precision_score(label_all, y_score_all)
plt.figure(figsize=(8, 6))
plt.plot(recall, precision, color='blue', lw=2, label='PR curve (AP = %0.4f)' % avg_precision)
plt.xlabel('Recall')
